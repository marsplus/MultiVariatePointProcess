...
void ConTinEst::GenerateCascade(std::set<unsigned>& initialSet, double TimeHorizon, std::map<unsigned, unsigned>& infectedBy,
							    std::vector<Node2Time>& cascade)
{
	...
	while (true) {
		...
		// in breadth-first search fashion to traverse the graph
		for (std::set<unsigned>::iterator u = G_->nodes[currentID].children.begin(); u != G_->nodes[currentID].children.end(); ++u) {

			// each node should be infected by only once. NO cycle
			...
			// if there is earlier infection took place, then
			// we only need to update infection time in visitedNodes
